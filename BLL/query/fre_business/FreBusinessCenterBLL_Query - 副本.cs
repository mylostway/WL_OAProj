            ...
            if (!string.IsNullOrEmpty(queryParam.ListID))
            {
                switch (queryParam.ListIDType)
                {
                    ...
                    case ListIDTypeEnums.WorkNo:
                        {
                            var query = session.QueryOver<FreBusinessBasicInfoEntity>().Where(x => x.Flist_id == queryListID1);
                            switch (queryParam.DateType)
                            {
                                ...
                                case DateTypeEnums.InputTime:
                                    {
                                        query.WhereifNotNull(queryStartDate, x => x.Finput_time >= queryStartDate.Value);
                                        query.WhereifNotNull(queryEndDate, x => x.Finput_time <= queryEndDate.Value);
                                        break;
                                    }
                            }
