...
namespace WL_OA.Data
{
        ...
        where TEnum : SmartEnum<TEnum, TValue>
    {
        ...
        {
            SAssert.MustTrue(!string.IsNullOrEmpty(name), string.Format("参数name不能为空"));
            var result = List.FirstOrDefault(item => string.Equals(item.Name, name, StringComparison.OrdinalIgnoreCase));
            if (result == null)
            {
                throw new Exception($"No {typeof(TEnum).Name} with Name \"{name}\" found.");
            }
            return result;
        }
        ...
        public static TEnum FromValue(TValue value)
        {
            ...
            if (result == null)
            {
                throw new Exception($"No {typeof(TEnum).Name} with Value {value} found.");
            }
            return result;
        }
